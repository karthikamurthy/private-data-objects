...
pdo_err_t SignupInfo::DeserializeSignupInfo(const std::string& serialized_signup_info)
{
    ...
    try
    {
        ...
        pdo::error::ThrowIfNull(
            parsed.value, "failed to parse serialized signup info; badly formed JSON");
        ...
        pdo::error::ThrowIfNull(
            pvalue, "invalid serialized signup info; missing enclave_persistent_id");
...
// XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
SignupInfo::SignupInfo(const std::string& serialized_signup_info)
...
static pdo_err_t DeserializePublicEnclaveData(
    const std::string& public_enclave_data, std::string& verifying_key, std::string& encryption_key)
{
    ...
    try
    {
        ...
        pdo::error::ThrowIfNull(
            parsed.value, "failed to parse the public enclave data, badly formed JSON");
    ...
    const std::string& originator_public_key_hash)
{
    ...
    presult = pdo::enclave_api::enclave_data::CreateEnclaveData(
        originator_public_key_hash, public_enclave_data, sealed_enclave_data, enclave_quote);
    ...
    presult =
        DeserializePublicEnclaveData(public_enclave_data.str(), verifying_key, encryption_key);
    ThrowPDOError(presult);
...
std::map<std::string, std::string> unseal_enclave_data(const std::string& sealed_enclave_data)
{
    ...
    presult =
        pdo::enclave_api::enclave_data::UnsealEnclaveData(sealed_enclave_data, public_enclave_data);
    ThrowPDOError(presult);
